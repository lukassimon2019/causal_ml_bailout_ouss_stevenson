...
gate_vars <- c("male", "defendantisblack", "Hisp", "White", "felony")

dml_gate_results <- list()

set.seed(1234) 

# Run GATE estimation for each outcome x gate_var combination
# (ATT is automatically computed as part of GATE estimation)
for(y in all_outcomes) {
  dml_gate_results[[y]] <- list()
  
  for(gate_v in gate_vars) {
    message("Running GATE for outcome: ", y, " | group: ", gate_v)
    
    dml_gate_results[[y]][[gate_v]] <- doubleML_did_cs_gate(
      data = CashBail,
      y_var = y,
...
## Summary Table
```{r}
# Extract ATT from first GATE variable results (they all have the same ATT)
final_dml_table <- do.call(rbind, lapply(all_outcomes, function(y) {
  res <- dml_gate_results[[y]][[gate_vars[1]]]  # Take ATT from first gate_var
  data.frame(
    Outcome = y,
...
```{r}
# Display GATE results for each outcome and grouping variable
for(y in all_outcomes) {
  cat("\n\n========================================\n")
  cat("OUTCOME:", y, "\n")
  cat("========================================\n")
  ...
  for(gate_v in gate_vars) {
    cat("\n--- Grouping by:", gate_v, "---\n")
    summary(dml_gate_results[[y]][[gate_v]])
  }
}
```
