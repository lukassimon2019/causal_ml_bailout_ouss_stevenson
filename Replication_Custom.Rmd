...
for(y in all_outcomes) {
  message("Running DML for outcome: ", y)
  
  dml_results_list[[y]] <- doubleML_did_cs(
    data = CashBail,
    y_var = y,
...
## Summary Table
```{r}
final_dml_table <- do.call(rbind, lapply(names(dml_results_list), function(name) {
  res <- dml_results_list[[name]]
  data.frame(
    Outcome = name,
