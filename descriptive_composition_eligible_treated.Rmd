...
cov_table <- CashBail %>%
  mutate(
    ...
    Group = ifelse(EligibleOffense == 1, "Eligible (Treatment)", "Ineligible (Control)")
  ) %>%
...
cov_table %>%
  ...
  tab_spanner(label = "Eligible (Treatment)", columns = 2:3) %>%
  tab_spanner(label = "Ineligible (Control)", columns = 4:5) %>%
  fmt_number(columns = 2:5, decimals = 2) %>%
  cols_label(
    Characteristic = "Defendant Characteristic",
    `Eligible (Treatment)_Pre-Reform` = "Pre",
    `Eligible (Treatment)_Post-Reform` = "Post",
    `Ineligible (Control)_Pre-Reform` = "Pre",
    `Ineligible (Control)_Post-Reform` = "Post"
  )
```
## Interpretation
